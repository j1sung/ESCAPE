...
bool AStar::Step(std::vector<std::vector<int>>& grid)
{
	// 더 이상 탐색할 수 없으면 종료.
	if (!isSearching)
	{
	...
	for (const Direction& direction : directions)
	{
		...
		// 기존 프로젝트처럼 먼저 찾는 과정을 '+'로 표시한다.
		SetCellState(grid, newX, newY, 5);
...
{
	Initialize(startPosition, goalPosition);
